...
/// Enables and resets peripheral clocks on various RCU registesr.
/// The first argument is a `apb1`, `ahb2` etc to specify the reg block. The second is something like
/// `tim1`, and the third is a `pac::RCU`.
#[macro_export]
macro_rules! rcu_en_reset {
    (apb1, $periph:expr, $rcu:expr) => {
        paste::paste! { cfg_if::cfg_if! {
            if #[cfg(any(feature = "f3"))] {
                $rcu.apb1en.modify(|_, w| w.[<$periph en>]().set_bit());
                $rcu.apb1rst.modify(|_, w| w.[<$periph rst>]().set_bit());
                $rcu.apb1rst.modify(|_, w| w.[<$periph rst>]().clear_bit());
            }
        }}
    };

    (addapb1, $periph:expr, $rcu:expr) => {
        paste::paste! { cfg_if::cfg_if! {
            if #[cfg(any(feature = "f3"))] {
                $rcu.addapb1en.modify(|_, w| w.[<$periph en>]().set_bit());
                $rcu.addapb1rst.modify(|_, w| w.[<$periph rst>]().set_bit());
                $rcu.addapb1rst.modify(|_, w| w.[<$periph rst>]().clear_bit());
            }
        }}
    };

    (apb2, $periph:expr, $rcu:expr) => {
        paste::paste! { cfg_if::cfg_if! {
            if #[cfg(feature = "f3")] {
                $rcu.apb2en.modify(|_, w| w.[<$periph en>]().set_bit());
                $rcu.apb2rst.modify(|_, w| w.[<$periph rst>]().set_bit());
                $rcu.apb2rst.modify(|_, w| w.[<$periph rst>]().clear_bit());
            }
        }}
    };
    
    (ahb, $periph:expr, $rcu:expr) => {
        paste::paste! { cfg_if::cfg_if! {
            if #[cfg(feature = "f3")] {
                $rcu.ahbenr.modify(|_, w| w.[<$periph en>]().set_bit());
            }
        }}
    };
}
...
//     fn en_reset(rcc: &RegisterBlock) {
//         rcu_en_reset!(apb1, tim7, rcc);
//     }
// }

impl RccPeriph for pac::I2C1 {
    fn en_reset(rcc: &RegisterBlock) {
        rcu_en_reset!(apb1, i2c1, rcc);
    }
}
...
impl RccPeriph for pac::SAI4 {
    fn en_reset(rcc: &RegisterBlock) {
        rcu_en_reset!(apb4, sai4, rcc);
    }
}

// impl RccPeriph for pac::USART1 {
//     fn en_reset(rcc: &RegisterBlock) {
//         rcu_en_reset!(apb2, usart1, rcc);
//     }
// }
...
//             fn en_reset(rcc: &RegisterBlock) {
//                 rcu_en_reset!(apb1, dac12, rcc);
//             }
//         }
//     } else if #[cfg(feature = "f3")] {
//         impl RccPeriph for DAC1 {
//             fn en_reset(rcc: &RegisterBlock) {
//                 rcu_en_reset!(apb1, dac1, rcc);
//             }
//         }
...
//         //     fn en_reset(rcc: &RegisterBlock) {
//         //         rcu_en_reset!(apb1, dac2, rcc);
//         //     }
//         // }
//     } else if #[cfg(feature = "g4")] {
//         impl RccPeriph for pac::DAC1 {
//             fn en_reset(rcc: &RegisterBlock) {
//                 rcu_en_reset!(ahb2, dac1, rcc);
//             }
//         }

//         impl RccPeriph for pac::DAC2 {
//             fn en_reset(rcc: &RegisterBlock) {
//                 rcu_en_reset!(ahb2, dac2, rcc);
//             }
//         }

//         impl RccPeriph for pac::DAC3 {
//             fn en_reset(rcc: &RegisterBlock) {
//                 rcu_en_reset!(ahb2, dac3, rcc);
//             }
//         }

//         impl RccPeriph for pac::DAC4 {
//             fn en_reset(rcc: &RegisterBlock) {
//                 rcu_en_reset!(ahb2, dac4, rcc);
//             }
//         }
