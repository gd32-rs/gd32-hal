...
macro_rules! rcc_en_reset {
    (apb1, $periph:expr, $rcc:expr) => {
        paste::paste! { cfg_if::cfg_if! {
            if #[cfg(any(feature = "f3"))] {
                $rcc.apb1en.modify(|_, w| w.[<$periph en>]().set_bit());
                $rcc.apb1rst.modify(|_, w| w.[<$periph rst>]().set_bit());
                $rcc.apb1rst.modify(|_, w| w.[<$periph rst>]().clear_bit());
            }
        }}
    };

    (addapb1, $periph:expr, $rcc:expr) => {
        paste::paste! { cfg_if::cfg_if! {
            if #[cfg(any(feature = "f3"))] {
                $rcc.addapb1en.modify(|_, w| w.[<$periph en>]().set_bit());
                $rcc.addapb1rst.modify(|_, w| w.[<$periph rst>]().set_bit());
                $rcc.addapb1rst.modify(|_, w| w.[<$periph rst>]().clear_bit());
            }
        }}
    };
    ...
    (apb2, $periph:expr, $rcc:expr) => {
        paste::paste! { cfg_if::cfg_if! {
            if #[cfg(feature = "f3")] {
                $rcc.apb2en.modify(|_, w| w.[<$periph en>]().set_bit());
                $rcc.apb2rst.modify(|_, w| w.[<$periph rst>]().set_bit());
                $rcc.apb2rst.modify(|_, w| w.[<$periph rst>]().clear_bit());
            }
        }}
    };
